...
aes3 = pygame.image.load(path.join(img_dir, "meteorBrown_big1.png")).convert()
aes2 = pygame.image.load(path.join(img_dir, "meteorBrown_big2.png")).convert()
enemy_spaceship = pygame.image.load(path.join(img_dir, "pixel_ship_red_small.png")).convert()
player_spaceship = pygame.image.load(path.join(img_dir, "pixel_ship_yellow.png")).convert()
laser_light = pygame.image.load(path.join(img_dir, "pixel_laser_red.png")).convert()
bg = pygame.transform.scale(pygame.image.load(path.join(img_dir, "background1.png")).convert(),(width,height))
powerup_img = pygame.image.load(path.join(img_dir, "bolt_gold.png")).convert()
...
def main():
    ...
    level = 0
    lives = 10
    main_font = pygame.font.SysFont('comicsans',20) #will use this font to draw
    ...
    def redraw_window(): #will redraw everytime loops run
        ...
        for powerup in powerups: #drawing powerups
            powerup.draw(win)


        powerups_label = main_font.render(f"Power-ups: {len(powerups)}", 1, (255, 255, 255))
        win.blit(powerups_label, (10, 40))
    ...
    while run:
        ...
        if random.randrange(0, 1000) == 0:
            powerup = PowerUp(random.randrange(50, width - 100), random.randrange(-1500, -100), powerup_img)
